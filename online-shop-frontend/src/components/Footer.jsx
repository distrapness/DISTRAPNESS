  ...
  <footer className="bg-black text-white pt-12 pb-8 mt-0 border-t border-gray-800">
    <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row md:justify-between md:items-start gap-8">
      ...
      <div className="flex-1 mb-8 md:mb-0 md:pr-8">
        <div className="text-xs font-bold uppercase mb-3 tracking-widest text-gray-300">Subscribe to our newsletter</div>
        <form className="flex flex-row gap-2 max-w-xs">
          <input
            type="email"
            placeholder="Email address*"
            className="px-3 py-2 rounded bg-black border border-gray-600 text-white placeholder-gray-400 text-sm w-full focus:outline-none focus:ring-2 focus:ring-white"
            required
          />
          <button type="submit" className="px-5 py-2 bg-white text-black font-bold rounded hover:bg-gray-200 text-sm">SUBSCRIBE</button>
        </form>
      </div>
      {/* Socials & Links */}
      <div className="flex-1 flex flex-col md:items-end gap-6">
        <div className="flex gap-4 mb-2">
          {socials.map(s => (
            ...
            >
              ...
              {s.icon === "shopee" && (
                // Shopee Bag Logo SVG (orange with S)
        ...
        <div className="flex flex-wrap gap-6 text-xs font-semibold tracking-wide uppercase text-gray-400 md:justify-end">
          <Link to="/how-to-order" className="hover:text-white">How to Order</Link>
          <a href="#" className="hover:text-white">Returns and Refunds</a>
        </div>
      </div>
    </div>
